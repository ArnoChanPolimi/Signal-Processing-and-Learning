...
%% %%%% ----------- RLS -----------------%%%%%%%
clear all;
% 输入：fs（采样率）, Sin_x（含噪立体声信号）, Sn_ref_x（参考噪声信号）
...
% 检查数据维度
% Sin_x: 含噪立体声信号 [左声道, 右声道]
% Sn_ref_x: 参考噪声信号（假设为单通道）
assert(size(Sin_x,2)==2, 'Sin_x应为立体声信号（双列）');
assert(size(Sn_ref_x,2)==1, 'Sn_ref_x应为单通道参考噪声');
...
for k = 1:signal_len
    ...
    if length(ref_idx) < N
        % 起始阶段填充零
        s_ref_vec = [zeros(N-length(ref_idx), 1); Sn_ref_x(ref_idx)]';
    else
        s_ref_vec = Sn_ref_x(ref_idx)';
    end
    
    % --- 左声道处理 ---
    % 计算先验误差
    e_left = Sin_x(k,1) - w_left' * s_ref_vec;
    ...
    % 去噪信号估计
    S_est(k,1) = Sin_x(k,1) - w_left' * s_ref_vec;
    
    % --- 右声道处理（同理）---
    e_right = Sin_x(k,2) - w_right' * s_ref_vec;
    ...
    S_est(k,2) = Sin_x(k,2) - w_right' * s_ref_vec;
end

%% 4. 播放音频对比
disp('播放原始含噪音频...');
sound(Sin_x, fs); pause(5);
disp('播放去噪后音频...');
sound(S_est, fs);
...
% 左声道对比
subplot(2,1,1);
plot(t, Sin_x(:,1), 'Color', [0.7 0.7 0.7], 'LineWidth', 1); hold on;
...
% 右声道对比
subplot(2,1,2);
plot(t, Sin_x(:,2), 'Color', [0.7 0.7 0.7], 'LineWidth', 1); hold on;
